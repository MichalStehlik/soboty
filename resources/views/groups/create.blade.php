...
@section('content')
<div class="container">
    <h1>Vytvoření nové skupiny</h1>
    <form class="form-horizontal" role="form" method="POST" action="{{ url('groups/create') }}">
        {{ csrf_field() }}
        <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }} row">
            <label for="name" class="col-md-4 control-label">Název</label>
            <div class="col-md-8">
                <input id="name" type="text" class="form-control" name="name" value="{{ old('name') }}" placeholder="Název skupiny" required autofocus>
        ...
        <div class="form-group{{ $errors->has('action') ? ' has-danger' : '' }} row">
            ...
            <div class="col-md-8">
                <select id="action" class="form-control" name="action" required autofocus>
                    ...
                    <option value="{{ $action->id }}" {{ (old('action',$targetAction) == "$action->id") ? "selected" : "" }}>{{ $action->year }} \ {{ $action->name }}</option>
                    @endforeach
                </select>
        ...
        <div class="form-group{{ $errors->has('lector') ? ' has-danger' : '' }} row">
            ...
            <div class="col-md-8">
                <select id="lector" class="form-control" name="lector" required autofocus>
                    ...
                    <option value="{{ $lector->id }}" {{ (old('lector',Auth::user()->id) == $lector->id) ? "selected" : "" }}>{{ $lector->firstname }} {{ $lector->lastname }}</option>
                    @endforeach
                </select>
        ...
        <div class="form-group{{ $errors->has('capacity') ? ' has-danger' : '' }} row">
            ...
            <div class="col-md-8">
                <input id="start" type="number" class="form-control" name="capacity" value="{{ old('capacity') }}" required autofocus>
        ...
        <div class="form-group{{ $errors->has('minimal_year') ? ' has-danger' : '' }} row">
            ...
            <div class="col-md-8">
                <select id="minimal_year" class="form-control" name="minimal_year" autofocus>
                    <option value="">-- vyberte --</option>
                    <option value="6" {{ (old('minimal_year') == "6") ? "selected" : "" }}>nižší než 7. třída ZŠ</option>
                    <option value="7" {{ (old('minimal_year') == "7") ? "selected" : "" }}>7. třída ZŠ</option>
                    <option value="8" {{ (old('minimal_year') == "8") ? "selected" : "" }}>8. třída ZŠ</option>
                    <option value="9" {{ (old('minimal_year') == "9") ? "selected" : "" }}>9. třída ZŠ</option>
                    <option value="10" {{ (old('minimal_year') == "10") ? "selected" : "" }}>vyšší třída (SŠ)</option>
                </select>    
        ...
        <div class="form-group row">
            <div class="col-md-8 offset-md-4">
                <div class="checkbox">
                    <label>
                        <input type="checkbox" name="open" {{ old('open') ? 'checked' : '' }}> Otevřená pro zápis
                    </label>
                </div>
            </div>
        </div>
